  ...
  return (
    <div 
      className={`
        ...
        transition-all duration-300 bar-josefin max-w-380 left-0
      `}
    >
        ...
        <div className="flex flex-col items-center justify-center flex-1 max-w-150">
          <div className="flex items-center w-full gap-4 md:gap-6 justify-center">
             ...
             <div className="hidden md:flex items-center gap-3 flex-1 w-full max-w-75 lg:max-w-100 select-none">
                <span className="text-xs font-medium text-gray-200 min-w-8.75 text-right">{fmtTime(currentTime)}</span>
                <div 
                  className="relative h-1.5 flex-1 bg-gray-400/50 rounded-full cursor-pointer group"
                  onClick={onBarClick}
                >
                  <div className="absolute top-0 left-0 h-full bg-[#38bdf8] rounded-full group-hover:bg-[#22d3ee] transition-colors" style={{ width: `${progressPercent}%` }}>
                ...
                <span className="text-xs font-medium text-gray-200 min-w-8.75">{fmtTime(duration)}</span>
             </div>
          </div>
        </div>
